...
export const login = {
  async loginAsync(
    ...
    const profile: ProfileConfig = await this._loadProfileAsync(profileName);
  ...
  async _askUserForRoleAndDurationAsync(
    ...
    aws_account_aliases: string | undefined
  ): Promise<{
    role: Role;
    ...
    if (roles.length === 0) {
      ...
      if (role) {
        ...
        let roleDisplayNames = _.map(roles, "roleArn");
        ...
        questions.push({
          ...
          message: "Role:",
          type: "list",
          choices: _.sortBy(roleDisplayNames),
    ...
    if (questions.length > 0) {
      ...
      if (!role) role = _.find(roles, ["roleArn", answers.role.split(" => ")[1]]);
